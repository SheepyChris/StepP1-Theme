...
t[#t+1] = Def.ActorFrame {
	...
	children = {
		LoadFont("_karnivore lite bold 20px")..{
			InitCommand=cmd(y,25;zoom,.67;maxwidth,230);
			RefreshCommand=function(self)
				local Group = SCREENMAN:GetTopScreen():GetCurrentGroup();
				self:settext( string.upper(RenameGroup(Group)) );
			end;
			OnCommand=cmd(stoptweening;playcommand,'Refresh';diffusealpha,0;sleep,.1;linear,.2;diffusealpha,1);
		...
		LoadActor( THEME:GetPathG("","ScreenSelectMusic/stage_channel.png") )..{
			...
			OnCommand=cmd(stoptweening;diffusealpha,0;sleep,.1;linear,.2;diffusealpha,1);
			OffCommand=cmd(stoptweening;diffusealpha,1;sleep,.1;linear,.2;diffusealpha,0);
...
t[#t+1] = Def.ActorFrame {
	...
	children = {
		LoadFont("_karnivore lite bold 20px")..{
			InitCommand=cmd(y,25;zoom,.67;maxwidth,200);
			RefreshCommand=function(self)
